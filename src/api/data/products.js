...
function _formatData(){
    ...
    _data.forEach(($item,$index)=>{
        let item = $item;
        item["id"] = $index; //Using index as ID ::: 
        item["key"] = $index;
        formatted[$index] = item;// Data now has key and other information
    });
    return formatted;//Returns complete formatted data of products :::: Object{} 
}
function _dataToArray(data){
return Object.values(data);//Returns an array []
}

export function getAllProducts(){
    return _dataToArray(_formatData());
}
export function getProductById(id){
   let found =  getAllProducts().filter((item)=> item.id == id);
   console.log("Searched Result ::: ",found);
   return found[0] ? found[0] : {};//Returns only one item ::: but since this is a mock server the items will be unique :::
}
